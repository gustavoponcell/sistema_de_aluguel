...
<<<<<<< HEAD
            ...
            transform_time = 0.0
            logger.info(
                "Agenda consultas: período=%s..%s status=%s status_lista=%s pagamento=%s busca=%s registros=%s hoje=%s tempo=%.3fs",
                self.start_date or "auto",
                self.end_date or "auto",
                self.status.name if isinstance(self.status, RentalStatus) else self.status,
                tuple(status.name for status in self.statuses) if self.statuses else "-",
                self.payment_status.name if isinstance(self.payment_status, PaymentStatus) else self.payment_status,
                self.search or "-",
                len(rentals),
                len(rentals_today),
                query_time,
...
<<<<<<< HEAD
    def _should_refresh(self, category: str) -> bool:
        if category == "global":
            ...
            return True
        if category in {"rentals", "payments", "inventory", "customers"}:
            self._last_data_change_ts = time.time()
            return True
        return False
...
<<<<<<< HEAD
        ...
        if status is None and not include_canceled:
            status_filters = VISIBLE_STATUSES_EXCLUDING_CANCELED
...
<<<<<<< HEAD
        ...
        if draft:
            message = QtWidgets.QMessageBox(self)
            message.setWindowTitle("Texto personalizado disponível")
            message.setText(
                "Existe um texto salvo pelo assistente "
                f"(atualizado em {draft.updated_at or 'data desconhecida'}). "
                "Deseja aplicá-lo a este documento?"
            )
            use_btn = message.addButton("Usar como está", QtWidgets.QMessageBox.AcceptRole)
            edit_btn = message.addButton(
                "Editar antes de usar", QtWidgets.QMessageBox.ActionRole
            )
            ignore_btn = message.addButton("Ignorar", QtWidgets.QMessageBox.RejectRole)
            cancel_btn = message.addButton("Cancelar", QtWidgets.QMessageBox.DestructiveRole)
            message.exec()
            clicked = message.clickedButton()
            if clicked == cancel_btn:
                return
            if clicked == use_btn:
                custom_terms = draft.text
            elif clicked == edit_btn:
                editor = DocumentTextDialog(
                    self,
