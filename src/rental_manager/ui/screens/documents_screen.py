...
<<<<<<< HEAD
    ...
    def _effective_period(self) -> tuple[Optional[str], Optional[str], bool]:
        ...
        end_qdate = QtCore.QDate.currentDate()
        start_qdate = end_qdate.addDays(-(DEFAULT_DOCUMENT_RANGE_DAYS - 1))
        return (
            start_qdate.toString("yyyy-MM-dd"),
            end_qdate.toString("yyyy-MM-dd"),
            True,
        )

    def _update_default_hint(self) -> None:
        self._default_range_hint.setVisible(not self._date_filter_check.isChecked())

    def _update_pagination_state(self) -> None:
        self._load_more_button.setVisible(self._has_more)
        self._load_more_button.setEnabled(self._has_more)

    def _on_load_more(self) -> None:
        self._load_documents(reset=False)

    def _load_documents(self, *, reset: bool = True) -> None:
        doc_type = self._type_combo.currentData()
        start_date, end_date, default_range = self._effective_period()
        ...
        try:
            ...
            rows = self._services.document_repo.list_documents(
                doc_type=doc_type,
            ...
            type_label = (
                doc_type.name if isinstance(doc_type, DocumentType) else str(doc_type or "-")
            )
            self._logger.info(
                (
                    "Documentos carregados: periodo=%s..%s tipo=%s busca=%s offset=%s "
                    "registros=%s tempo=%.3fs"
                ),
                start_date or "-",
                end_date or "-",
                type_label,
            ...
            self._logger.exception("Falha ao carregar documentos.")
        ...
        self._documents.extend(page_rows)
        self._current_offset += len(page_rows)
        self._default_range_active = default_range
    ...
    def _update_status_label(self) -> None:
        if not self._documents:
            if self._default_range_active:
                self._status_label.setText(
                    f"Nenhum documento encontrado nos ̼ltimos {DEFAULT_DOCUMENT_RANGE_DAYS} dias."
                )
            else:
                self._status_label.setText("Nenhum documento encontrado.")
            return
        range_hint = (
            f" (̼ltimos {DEFAULT_DOCUMENT_RANGE_DAYS} dias)"
            if self._default_range_active
            else ""
        )
        self._status_label.setText(
