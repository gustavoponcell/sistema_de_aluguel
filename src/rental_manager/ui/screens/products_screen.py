...
class ProductsScreen(BaseScreen):
    ...
    def _render_table(self, products: List[Product]) -> None:
        ...
        status_map: dict[int, dict[str, Optional[int]]] = {}
        if product_ids:
            fetch_start = time.perf_counter()
            try:
                status_map = self._services.inventory_service.get_bulk_status_on_date(
                    product_ids,
                    reference_date,
                )
            except Exception:
                self._logger.exception("Falha ao calcular disponibilidade em lote.")
                status_map = {}
            else:
                elapsed = time.perf_counter() - fetch_start
                self._logger.info(
                    "Estoque em lote para %s itens calculado em %.3fs",
                    len(product_ids),
                    elapsed,
                )
        self.table.setRowCount(len(products))
        for row, product in enumerate(products):
            status = status_map.get(product.id or 0, {})
            is_service = product.kind == ProductKind.SERVICE
            is_sale = product.kind == ProductKind.SALE
            reserved_qty = status.get("reserved")
            available_qty = status.get("available")
            if is_service:
                reserved_display = "—"
                available_display = "Sem controle de estoque"
                total_display = "—"
            else:
                total_display = str(product.total_qty)
                if is_sale:
                    reserved_display = "—"
                else:
                    reserved_display = str(reserved_qty if reserved_qty is not None else 0)
                available_value = (
