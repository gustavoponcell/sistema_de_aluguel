...
<<<<<<< HEAD
    def open_settings_screen(self) -> None:
        ...
        if self._settings_index is None:
            return
        self._select_screen_by_index(self._settings_index)

    def open_new_rental_screen(self) -> None:
        self._select_screen_by_label("Novo Pedido")

    def _select_screen_by_label(self, label: str) -> None:
        index = self._screen_indexes.get(label)
        if index is None:
            return
        self._select_screen_by_index(index)

    def _select_screen_by_index(self, index: int) -> None:
