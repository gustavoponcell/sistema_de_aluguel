...
**Componentes**
- Checkbox **Habilitar Fluxos Inteligentes**.
- Campo de texto multilinha para **Mensagem exibida quando os fluxos estiverem desativados**.
- Bot?o **Salvar prefer?ncias** e r?tulo de status.

**Regras e comportamento**
- Por padr?o os fluxos ficam habilitados (`assistant.flows_enabled = true`).
- Se o usu?rio desativar, o banner da tela do assistente troca para ?Fluxos desativados? e mostra a mensagem configurada.
- Nenhuma chamada externa ? feita; tudo opera com dados locais.

**Novos campos no config (`assistant`):**
- `flows_enabled` (bool).
